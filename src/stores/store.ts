 ...
 *  +-----------------------+--------------------------------------------+--------------------------------------------------+-------------------------------------------------+--------------------------+----------------+---------------------------------------------------------------+
 *  | Biblioteca / Patrón   | Características                            | Diferencias                                      | Ventajas                                        | Facilidad de aprendizaje | Demanda laboral| Casos de uso                                                  |
 *  +-----------------------+--------------------------------------------+--------------------------------------------------+-------------------------------------------------+--------------------------+----------------+---------------------------------------------------------------+
 *  | Redux                 | Estado global predecible;                  | Arquitectura Flux;                               | Mantenimiento en apps grandes;                  | ★★☆☆☆ (2/5)            | ★★★★☆ (4/5)  | Apps grandes con lógica compleja, aplicaciones empresariales. |
 *  +-----------------------+--------------------------------------------+--------------------------------------------------+-------------------------------------------------+--------------------------+----------------+---------------------------------------------------------------+
 ...
 *  +-----------------------+--------------------------------------------+--------------------------------------------------+-------------------------------------------------+--------------------------+----------------+---------------------------------------------------------------+
 *  | React State + Context | Estado local `useState` y `Context` para   | Integrado en React; sin middleware por defecto;  | Muy simple para casos pequeños/medios;          | ★★★★★ (5/5)            | ★★☆☆☆ (2/5)  | Apps pequeñas o medianas, prototipos, componentes aislados,   |
 *  |                       | compartir datos.                           | no requiere normalización.                       | sin dependencias; directo.                      |                          |                | compartir temas o autenticación.                              |
 *  +-----------------------+--------------------------------------------+--------------------------------------------------+-------------------------------------------------+--------------------------+----------------+---------------------------------------------------------------+
 */


// create: Es la Funcion principal de Zustand. Permite crear un store para manejar el estado global de la aplicación.
import { create } from 'zustand';

// Interfaz que define la estructura del estado (en esta caso es un contador) y las acciones (setters) para modificarlo.
// PD: Pueden definirse tantas interfaces como estados y acciones se requieran en la aplicación.
